        ...
        <div class="p-3 flex-1 flex flex-col sm:flex-row">
            ...
            <div class="flex flex-1 min-w-0">
                <div class="flex flex-shrink-0 items-center p-3" @click.stop>
                    <Checkbox :checked="selected" @update:checked="$emit('select', post)"
            ...
            <div class="absolute right-4 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-2 p-1 z-20">
                <div class="p-1.5 hover:bg-primary/10 rounded-md cursor-pointer text-muted-foreground hover:text-foreground transition-colors"
                    @click.stop="$emit('preview', post)" title="预览">
                    <EyeIcon class="size-3" />
                </div>
                <div class="p-1.5 hover:bg-primary/10 rounded-md cursor-pointer text-muted-foreground hover:text-foreground transition-colors"
                    @click.stop="$emit('edit', post)" title="编辑">
                    <PencilIcon class="size-3" />
                </div>
                <div class="p-1.5 hover:bg-destructive/10 hover:text-destructive rounded-md cursor-pointer text-muted-foreground transition-colors"
                    @click.stop="$emit('delete', post)" title="删除">
